...
def build_pdf(r):
    ...
    e.append(Paragraph("Output &mdash; final norm + LM head", H2))
    e.append(Paragraph("Normalize, then project the last token's vector onto the vocabulary and softmax. "
        "<i>(Illustrative untrained head over a small demo vocabulary.)</i>", BODY))
    rows = [["token", "logit", "prob"]] + [[v, f"{l:.2f}", f"{p:.2f}"] for v, l, p in
            zip(r['vocab'], r['logits'], r['probs'])]
    t = Table(rows, hAlign='LEFT', colWidths=[80, 60, 60])
    t.setStyle(TableStyle([('FONT', (0, 0), (-1, -1), 'Helvetica', 8),
        ('FONT', (0, 0), (-1, 0), 'Helvetica-Bold', 8), ('TEXTCOLOR', (0, 0), (-1, 0), INDIGO),
        ('LINEBELOW', (0, 0), (-1, 0), 0.5, SLATE), ('BOTTOMPADDING', (0, 0), (-1, -1), 2)]))
    e.append(t)
    top = r['topk'][0]
    e.append(Paragraph(f"<b>Most likely next token (demo): &ldquo;{top[0]}&rdquo; at {top[1]*100:.0f}%.</b> "
        "The token is appended and the loop repeats &mdash; autoregression.", BODY))
